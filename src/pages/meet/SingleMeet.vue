...
export default defineComponent({
  ...
  methods: {
    ...
    toMeet(time, room, day) {
      ...
      console.log('singleMeet toMeet temp_hover_start_time - temp_hover_end_time',temp_hover_start_time,temp_hover_end_time)
      ...
      if (min_result_start === MRBS_MAX && min_result_end === MRBS_MAX) {
        console.log('判断当前列是否没有会议')
      }
      // 3、判断当前会议选择是否为同一场会议
      else if (min_entry_start_id === min_entry_end_id) {
        console.log('判断当前会议选择是否为同一场会议')
        ...
        console.log('在二个会议之间的hover（计算出来的最低结束会议时间、最低开始会议时间）')
      ...
      console.log('singleMeet toMeet min_start - min_end',temp_min_start_time,temp_min_end_time)
      ...
      console.log('singleMeet toMeet temp_start_time - temp_end_time',temp_start_time,temp_end_time)
      console.log('singleMeet toMeet hover_start_time - hover_end_time:',moment.tz(hover_start_time * 1000, 'Asia/Shanghai').format('HH:mm'),moment.tz(hover_end_time * 1000, 'Asia/Shanghai').format('HH:mm'))
      console.log('singleMeet toMeet cal_hover_start_time - cal_hover_end_time',cal_hover_start_time,cal_hover_end_time)
      // 间隔少于5分钟不可以编辑
      if ((cal_hover_end_time - cal_hover_start_time) / 60 < 5) {
        return
      }
      this.addParams.cal_start_time = cal_hover_start_time
      this.addParams.cal_end_time = cal_hover_end_time
