  ...
  data() {
    return {
      ...
      questionList:[
        ...
        {title: '6、误删除了用户怎么办？',reply: '答：先确认用户是ad导入还是系统内创建的用户，如果是ad导入的你可以在系统设置里的用户同步手动同步一次或等下次自动同步，如果是系统创建的则需要手动再次创建'},
        {title: '7、为什么会议室不可以预定？',reply: '答：请先使用管理员账号在区域&会议室管理界面查看当前区域或会议室的状态是否正常'},
        {title: '8、为什么会议室状态正常也无法预定？',reply: '答：如果确定会议室状态正常，你可以确认下当前会议室所属区域的状态是否正常，以及你是否存在预定当前会议室的权限'},
        {title: '9、为什么我有当前区域的权限却无法创建区域下的会议室？',reply: '答：当区域下的会议室没有做具体组的权限时，你可以创建该区域下所有的会议室，但是如果会议室创建时配置了组的权限那么已会议室配置优先'},
        ...
        {title: '12、怎么把终端和会议室解除绑定？',reply: '答：如果你只是在平板端切换会议室，你可在平板端设置中切换，如果是解绑可以在“终端设备管理”界面解绑，但是注意解绑后平板端会立马处理原始待激活状态'},
        {title: '13、在全局会议规则设置里关闭了会议信息展示为什么还会显示？',reply: '答：这里的展示开关为创建会议室时的默认状态开关，实际会议信息开关需要到会议室编辑里改动'},
        {title: '14、未找到需要的查询问题？',reply: '答：你可以先查看下“用户操作手册”，如果还未解决您的问题可在“问题反馈”中提交您的问题'},
      ]
    }
  },
  methods: {
